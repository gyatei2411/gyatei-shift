...
4. データベース作成後、上の **ルール** タブを開いて以下を貼り付け → **公開**
   ...
   - `requests` の **直下に `".read": true`** が必要。
     `board.html`（タブレット画面）は全週の一覧をまとめて読むため、
     `$reqId` の中だけに `.read` を書いても**権限エラーになる**（ルールは下にしか伝播しない）
   - `staffmeta` は「できるポジション（0部/1部/2部）」「★優先」「個人メモ」の保存先。
     これを書かないと、それらの設定は**その端末のブラウザ内だけ**に保存され、他の端末と共有されません
   - 確定シフト・修正履歴は `requests/{reqId}/confirmed` に入るので、上記ルールでカバーされる
...
- 上段の1〜2列目（始業・終業）と下段の1〜2列目（**0部**・メインポジション）は管理者が後で手書き
  - 下段の1列目（ポジションの左）＝ **0部（9:00〜9:30 朝の準備）** … C / 1F / Rj / Tj / m
- 末尾の備考行に全体備考を集約
...
シフト表画面の **✅ シフトを確定** を押すと、その時点の内容が記録され、タブレットに表示されるようになります。

- 確定後もセルを編集できます。編集するとバッジが「⚠ 未確定の修正あり」に変わります
- **🔄 修正を確定** を押すと v2, v3… とバージョンが上がり、変更内容が履歴に残ります
- **📜 履歴** で「いつ・誰の・何曜日を・どう変えたか」を見ることができます

例: `8/17(月) 青木: K 9-L → R2 930-16`

### タブレットで見る

タブレットのブラウザで以下を開き、ホーム画面に追加してください。

```
https://<ユーザー名>.github.io/gyatei-shift/board.html
```

- 今週の確定シフトが自動で選ばれ、**1画面に1週間分**が入るよう拡大されます
- 管理画面で修正を確定すると、タブレット側も自動で切り替わります（Firebase接続時）
- ◀ ▶ で前後の週も見られます
- この画面は**編集できません**ので、スタッフに見せても安全です
